 ...
 * BUGS: Doesn't work on graphs with edges of weight zero. (I'm not sure if its suppsed to). Have assert(w>0) while reading in graph to catch and stop program.
...
struct GraphEdge{ 
    int x, y;
    double w; // changed w to be a double
    void readinput(){
        scanf("%d%d%lf", &x, &y, &w); // changed scanf to expect a double
        assert(w > 0); // If a non-positive value is entered as weight, quit!
    }
...
int main()
{
    while(1)
    {
        ...
        // New Code: (almost) everything above was provided in Practicum
