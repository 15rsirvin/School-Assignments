...
{
    // your code starts here
    int curr = (left_index + right_index )/ 2;
    string currTerm = terms[curr].query;
    
    if (currTerm.substr(0,key.length()) == key)
        return curr;
    ...
    else if(currTerm  > key)
    {
        return binary_searchHelper(terms, key, left_index, curr -1);
    }

    else
    {
        return binary_searchHelper(terms, key, curr + 1, right_index);
    }
...
void Autocomplete::search(string key, int& firstIndex, int& lastIndex)
{
    ...
    else
    {
        ...
        // looking forwards
        while (i >= 0 && Term::compareByPrefix(terms[found_pos],terms[i -1], r) == 0)
        {
            i--;
        }
        firstIndex = i;
        ...
        // looking backwards;
        i = found_pos;
        while(i < terms.size() && Term::compareByPrefix(terms[found_pos],terms[i + 1], r) == 0)
        {
            i++;
        }
        lastIndex = i;
...
SortingList<Term> Autocomplete::allMatches(string prefix)
{
    ...
    int (*sortWeight)(Term, Term) = &Term::compareByWeight;
    match.bubble_sort(*sortWeight);
