...
int main()
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            {
                cout << "The distance from source vertex " << source << " to vertex " << i 
                     << " is " << distance[i] << "-edge away." << endl
                     << "the path back to the source vertex:" << endl;
                ...
                while ( curr != source)
                {
                    cout << " ---> " << go_through[curr];
                    curr = go_through[curr];
                }
                cout << endl;

            }

            for(int i = source + 1; i < VERTICESNUM; i++)
            {
                cout << "The distance from source vertex " << source << " to vertex " << i 
                     << " is " << distance[i] << "-edge away." << endl
                     << "the path back to the source vertex:" << endl;
